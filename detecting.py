...
def orientation(p,q,r):
    #finding slope of the line to decide orientation 
    #Slope of line segment (p1, p2): σ = (y2 - y1)/(x2 - x1)
    #Slope of line segment (p2, p3): τ = (y3 - y2)/(x3 - x2)
    val = int(((q[1] - p[1]) * (r[0] - q[0])) - ((q[0] - p[0]) * (r[1] - q[1]))) #(y2 - y1)*(x3 - x2) - (y3 - y2)*(x2 - x1)
    if val == 0:
        #linear
        return 0
    elif (val>0):
...
bg = Image.open("SnakeGame.png")
new_img = bg.resize((1366,768))
bg1 = ImageTk.PhotoImage(new_img)

start_img = Image.open("start_game.jpeg")
new_img = start_img.resize((170,50))
start_image = ImageTk.PhotoImage(new_img)

exit_img = Image.open("end_game.jpeg")
...
btn = Snake_Game.Button(wnd, image=start_image, height =50, width=170, bd = 0, relief = "flat" ,command = on_click)
btn.pack()
btn.place(relx=0.20,rely=0.40)

btn = Snake_Game.Button(wnd, image=exit_image, height =50, width=170, bd = 0, relief = "flat" ,command = wnd.destroy)
btn.pack()
btn.place(relx=0.20,rely=0.50)
